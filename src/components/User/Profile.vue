<template>
	...
	<div class="wrapper">
		<div class="header">
			...
			<div class="item center">{{isMySelf?'我的资料':'个人资料'}}</div>
			<div class="item right">
				<template v-if="isMySelf">
					<img src="./icon/qq_setting_me_qr_code_icon.png" alt="">
				</template>
		...
		<div class="profile">
			...
			<div class="profile_face">
				<img :src="dataList.face" alt="">
				<p class="nickname">{{isMySelf?userInfo.nick_name:dataList.beizhu}}</p>
				<p class="sign">{{dataList.signature}}
					<img src="./icon/qq_profilecard_signature_more.png" alt="">
				</p>
			</div>
			<div class="profile_info">
				<ul>
					<li class="account">{{dataList.nick_name}}({{dataList.qq}})</li>
					<li class="information">
						<span>{{dataList.sex}}</span>
						<span>{{dataList.age}}岁</span>
						<span>{{dataList.xingzuo}}</span>
						<span>{{dataList.place}}</span>
						<img src="./icon/qq_profilecard_signature_more.png" class="right" alt="">
					</li>
					<li class="profession">
						{{dataList.favor==''?'TA还没有兴趣爱好':dataList.favor}}
					</li>
					<li class="rank">
						<img src="./icon/hni.png" class="vip" v-if="dataList.vip==0" alt="">
						<img src="./icon/vip.png" class="vip" v-if="dataList.vip==1" alt="">
						<img src="./icon/ewm.png" class="vip" v-if="dataList.vip==2" alt="">
						<img src="./icon/hdr.png" class="level" v-for="n in level.crown" alt="">
						<img src="./icon/hds.png" class="level" v-for="n in level.sun" alt="">
						<img src="./icon/hds.png" class="level" v-for="n in level.star" alt="">
					...
					<li class="qzone">
						{{isMySelf?userInfo.nick_name:dataList.beizhu}}的空间
						<img src="./icon/qq_profilecard_signature_more.png" class="right" alt="">
					</li>
				</ul>
			</div>
	...
	export default{
		...
		methods:{
			...
			async getUserProfile(userId,targetUserId){
				console.log("isMySelf:",this.isMySelf);
				if(this.isMySelf){
					this.dataList = this.userInfo;
					return;
				}
				//const {data} = await get_user_profile(userId,targetUserId);
				const data = {"id":2,"user_id":2,"nick_name":"浅唱低吟","signature":"路漫漫其修远兮","face":"/static/user/face/4.jpg","sex":"女","age":19,"xingzuo":"天蝎座","place":"江西","favor":"娱乐/艺术/表演","level":30,"profile_bg":"/static/user/bg/3.jpg","login_day":0,"vip":2,"accert":"1.4倍加速中","qq":"986992483","beizhu":"楚乔","isFriend":1};
				this.dataList = data;
