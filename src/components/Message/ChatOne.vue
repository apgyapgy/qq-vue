	...
	<div class="wrapper">
		<div class="header">
			...
			<div class="item center">
				...
				<span class="status">{{dataList.stauts}}</span>
			</div>
			<div class="item right">
				<img src="./icon/skin_aio_head_twocall.png" class="phone" alt="">
				<img src="./icon/skin_header_icon_single.png" alt="" class="person" @click="$router.push(`/chat_one/${dataList.user_id}/set`)">
			</div>
		</div>
...
<script>
	...
	//import * as api from '@/api/chat';//写接口后再取消注释
		...
		methods:{
			...
			async sendMessage(){
				...
				let data = {
					...
					tiem:Date.parse(new Date())/1000 //时间
				}
			...
			addMessageLocal(data){
				const message = this.dataList.message;//获取本地已存在 的消息
				if(message.length == 0){
			...
			updateBySocket(){
				...
				//一定要先移除原来的事件，否则会有重复的监听器

				socket.on('receivePrivateMessage',(data) => {
					if(data.from_suer == this.userId){
						return;
					}
					...
					this.$store.commit('UPDATE_MESSAGE',{
						...
						time:data.time,
						type:'single',
						isEnterChat:this.dataLsit.user_id==data.from_user //是否进入聊天页面，进入了的话那么该条消息的unread就是0
					});
				});
			...
			sendBySocket(){
				socket.emit('sendPrivateMessage',{
					...
					from_suer_face:this.userInfo.face,
					from_user_beizhu:this.dataList.beizhu,
					to_user:this.dataList.user_id,
