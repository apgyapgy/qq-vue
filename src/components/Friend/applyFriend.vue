		...
		<div class="apply">
			<ul>
				<li @click="$router.push(`${dataList.user_id}/profile`)">
					<img :src="dataList.face" alt="" class="user">
					<div class="info">
						<p class="name">{{dataList.nick_name}}</p>
						<p class="message">
							<span class="sex">{{dataList.sex}}</span>
							<span>{{dataList.age}}岁</span>
							<span>{{dataList.face}}</span>
						</p>
						<img src="./icon/nqj.9.png" alt="" class="icon">
					</div>
				</li>
			</ul>
			<div class="other">
			...
			<div class="actions">
				<button class="reject" @click="reject(apply_id)" v-if="dataList.status==1">拒绝</button>
				<button class="agree" @click="agree(apply_id)" v-if="dataList.status==1">同意</button>
