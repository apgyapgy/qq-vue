<template>
	...
	<div class="wrapper">
		...
		<VScroll :data="dataList.friends" :isHeightChange="isHeightChange" component="friend" :tabIndex="tabIndex">
			...
			<div class="tab">
				<div class="item first" :class="{'blue':tabIndex==1}" @click="changeTab(1)">好友</div>
				<div class="item" :class="{'blue':tabIndex==2}" @click="changeTab(2)">群</div>
				<div class="item" :class="{'blue':tabIndex==3}" @click="changeTab(3)">多人聊天</div>
				<div class="item" :class="{'blue':tabIndex==4}" @click="changeTab(4)">设备</div>
				<div class="item" :class="{'blue':tabIndex==5}" @click="changeTab(5)">通讯录</div>
				<div class="item" :class="{'blue':tabIndex==6}" @click="changeTab(6)">公众号</div>
			</div>
			<div class="main">
				<!-- 好友 -->
						...
						<li class="li-2" v-show="status.friend[index]==1">
							<ul class="level-2">
								<li v-for="member in item.members" @click="chatOne(member.id)">
									<img :src="status.friend[index]==1?member.face:''" class="user" :class="{'offline':member.status=='离线'}" alt="">
									<div class="info">
										<p class="from" :class="{'vip':member.vipurl!=''&&member.status!='离线'}">{{member.name}}</p>
										<img :src="status.friend[index]==1?member.vipurl:''" v-if="member.vipurl!=''" alt="">
				...
				<div class="groups" v-show="tabIndex==2">
					...
					<ul class="level-1" v-for="(item,index) in dataList.groups">
						<li class="li-1" @click="toggle(index,2)" :class="{'click':status.group[index]==1}">{{item.type}}
						...
						<li class="li-2" v-show="status.group[index]==1">
							<ul class="level-2">
								<li v-for="groups in item.groups" @click="chatGroup(groups.id)">
									<img :src="status.group[index]==1?groups.url:''" class="user" alt="">
				...
				<div class="chats" v-show="tabIndex==3">
					...
					<ul class="level-2">
						<li v-for="item in chats">
							<img src="./icon/search_group.png" class="user" alt="">
							<div class="info">
								{{item.name}}
				...
				<div class="device" v-show="tabIndex==4">
					<ul class="level-2">
						<li>
							<img src="./icon/feb.png" alt="" class="user">
							<div class="info">
								<p>我的电脑</p>
						...
						<li>
							<img src="./icon/oxy.png" alt="" class="user">
							<div class="info">
								<p>我的打印机</p>
						...
						<li>
							<img src="./icon/nrg.png" alt="" class="user">
							<div class="info">
								<p>发现新设备</p>
				...
				<div class="address-list" v-show="tabIndex==5">
					<ul class="level-2">
						<li v-for="item in addressList">
							<img :src="tabIndex==5?item.face:''" alt="" class="user">
							<div class="info">
								<p>{{item.name}}</p>
					...
					<div v-for="item in officialAccount">
						...
						<ul class="level-2">
							<li v-for="account in item.accounts">
								<img :src="tabIndex==6?account.url:''" alt="" class="user">
