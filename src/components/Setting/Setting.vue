<template>
	...
	<div class="wrapper">
		...
		<VScroll component="setting">
			<div class="Sysconfig">
				<div class="first">
					<p class="item" @click="$router.push('/accont_set')">帐号管理
						<span class="face">
							<img :src="dataList.face" alt="">
						</span>
					</p>
					<p class="item">手机号码
						<span class="phone">{{formatPhone}}</span>
					</p>
					<p class="item">
						QQ达人
						<span class="da">
							<img src="./icon/poa.png" alt="">{{dataList.time}}天
						</span>
					</p>
				</div>
				<div class="second">
...
<script>
	...
	export default{
		...
		computed:{
			...
			formatPhone(){
				return `${this.dataList.phone.substring(0,3)}****${this.dataList.phone.substring(7,11)}`;
			}
		},
		components:{
			VScroll
